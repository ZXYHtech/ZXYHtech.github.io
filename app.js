...
async function protectedDownload(path,filename){try{const response=await fetch(path,{headers:{'Authorization':`Bearer ${state.adminToken}`}});if(!response.ok)throw new Error('æ²¡æœ‰æƒé™å¯¼å‡ºè¯¥æ–‡ä»¶');const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=filename;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);return true}catch(err){toast(err.message);return false}}
...
function renderCart() {
  ...
  </div>`}).join('') + `<div class="cart-total"><span>å‚è€ƒåˆè®¡</span><b>${money(total)}</b></div><button class="button primary" id="checkout-button" style="width:100%">æ ¸å¯¹å¹¶æäº¤éœ€æ±‚</button><p class="cart-inventory-note">åº“å­˜æ¥è‡ªå½“å‰åº“å­˜é€‚é…å™¨ï¼›æ•°é‡ä¸è¶³ä»å¯æäº¤éœ€æ±‚ï¼Œç”±é”€å”®ç¡®è®¤è¡¥è´§å’Œäº¤æœŸã€‚</p>`;
}
function renderCheckoutSummary(){
  ...
  summary.innerHTML=`<div class="checkout-summary-head"><b>æœ¬æ¬¡è´­ä¹°æ¸…å•</b><span>${quantity} ä»¶å•†å“</span></div>${state.cart.map(item=>{const stock=state.availability[item.product_id],available=Number(stock?.quantity||0),enough=stock&&available>=item.quantity,status=!stock?'åº“å­˜å¾…æ ¸å¯¹':enough?`ç°è´§ ${available} ä»¶ï¼Œå¯æ»¡è¶³æœ¬æ¬¡éœ€æ±‚`:available?`ç°è´§ ${available} ä»¶ï¼Œç¼ºå°‘ ${item.quantity-available} ä»¶`:'æš‚æ— ç°è´§ï¼Œéœ€ç¡®è®¤äº¤æœŸ';return `<div class="checkout-summary-item ${stock&&!enough?'stock-warning':''}"><span><b>${escapeHtml(item.model)}</b><small>${escapeHtml(item.name)}</small><small class="checkout-stock">${escapeHtml(status)}</small></span><span>Ã— ${item.quantity}</span><strong>${money(item.price*item.quantity)}</strong></div>`}).join('')}<div class="checkout-summary-total"><span>å‚è€ƒåˆè®¡</span><b>${money(total)}</b></div>`;
}
...
function closeCart() { $('#ãN|îÚ$z{-®éÜj×ff÷&—FRæ6Æ÷6W7B‚rç&öGV7BÖ6&Br“·FövvÆTff÷&—FR†ff÷&—FRæFF6WBæff÷&—FR“¶–b‡&VÖ÷fTg&öÔff÷&—FUf–Wrbf6&B—¶6&Bç&VÖ÷fR‚“¶–b‚B‚r7&öGV7BÖw&–Bç&öGV7BÖ6&Br’’B‚r7&öGV7BÖw&–Br’æ–ææW$…DÔÃÒsÆF—b6Æ73Ò&V×G’#î‹ùk*iÈiKn‰xşzÊnYiÚK»ny¨NKª~Y8ÂöF—câw×&WGW&çĞ¢6öç7BFCÖRçF&vWBæ6Æ÷6W7B‚u¶FFÖFBÖ6'EÒr“²–b†FB’FEFô6'B†FBæFF6WBæFD6'B“°¢6öç7B6ö×&SÖRçF&vWBæ6Æ÷6W7B‚u¶FFÖ6ö×&UÒr“²–b†6ö×&R—·FövvÆT6ö×&R†6ö×&