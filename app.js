...
if ('serviceWorker' in navigator && location.protocol.startsWith('http')) {
  addEventListener('load',()=>navigator.serviceWorker.register(sitePath('service-worker.js?v=1.29.6'),{updateViaCache:'none'}).catch(()=>{}));
}
...
function relatedProductsFor(product){
  const preferred=(PRODUCT_RELATIONS[product.slug]||[]).map(slug=>state.products.find(item=>item.slug===slug)).filter(Boolean);
...
function ecosystemLinks(product){
  ...
  return `<section class="product-ecosystem" data-review-id="product.${escapeHtml(product.slug)}.ecosystem"><div class="section-head"><div><span class="eyebrow">PRODUCT ECOSYSTEM</span><h2>控制器、模块与上位机协同使用</h2></div><p>相关入口按实际组合关系连接</p></div><div class="ecosystem-grid">
    ...
    <a href="${sitePath('zya1000-console.html?v=1.29.6')}" target="_blank" rel="noopener"><span>WEB CONSOLE</span><b>ZYA1000 网页上位机</b><p>设备发现、多设备同步、补偿数据、日志和自动化测试。</p><i>打开网页版上位机 →</i></a>
...
function controllerEntryHub(product){
  ...
  return `<section class="controller-entry-hub" data-review-id="product.zyc100.controller-center">
    ...
    <div class="controller-entry-grid">
      <a class="primary-entry" href="${sitePath('zya1000-console.html?v=1.29.6')}" target="_blank" rel="noopener"><span>WEB APP</span><b>打开网页版上位机</b><p>连接 USB CDC，控制衰减、多设备同步并执行自动化时间线。</p><i>进入全功能控制台 →</i></a>
      <a href="${sitePath('zya1000-console.html?v=1.29.6&embed=compact')}" target="_blank" rel="noopener"><span>QUICK CONTROL</span><b>快速衰减控制</b><p>只保留衰减值和实时通信日志，适合现场快速调整。</p><i>打开紧凑模式 →</i></a>
...
function controllerConsoleEmbed(product){
  ...
  return `<section class="controller-console-inline" data-review-id="product.zyc100.web-console">
    <div class="section-head"><div><span class="eyebrow">WEB SERIAL CONTROL</span><h2>网页衰减控制</h2><p>连接控制器后，仅显示衰减控制和实时通信日志。</p></div><a class="button primary" href="${sitePath('zya1000-console.html?v=1.29.6')}" target="_blank" rel="noopener">进入全屏上位机 →</a></div>
    <iframe data-zya-compact src="${sitePath('zya1000-console.html?v=1.29.6&embed=compact')}" title="ZYA1000 快速衰减控制" allow="serial" loading="lazy"></iframe>
  </section>`;
}
...
function pagesPublishPanel(pages){const snapshot=pages.snapshot||{};return `<section class="panel pages-publish-panel"><div class="admin-toolbar"><div><span class="eyebrow">PAGES CONTENT PUBLISHER</span><h3>发布商品目录静态快照</h3><p>将当前已启用商品、说明书、教程和资源索引写入 Pages 仓库；访客在线时仍优先读取服务器。</p></div><span class="status-tag ${pages.publish_ready?'on':'warn'}">${pages.publish_ready?'可以发布':'仅可预览'}</span></div><div class="pages-publish-meta"><span><b>${snapshot.products??0}</b> 个商品</span><span><b>${snapshot.documents??0}</b> 份文档</span><span><b>${snapshot.resources??0}</b> 个资源</span><span><b>${formatBytes(snapshot.bytes||0)}</b> 快照大小</span></div><dl><div><dt>目标仓库</dt><dd>${escapeHtml(pages.repository)}</dd></div><div><dt>分支与路径</dt><dd>${escapeHtml(pages.branch)} · ${escapeHtml(pages.path)}</dd></div><div><dt>内容指纹</dt><dd><code>${escapeHtml((snapshot.sha256||'').slice(0,16))}</code></dd></div></dl><div class="modal-actions"><button type="button" class="button ghost" id="pages-publish-preview">重新生成预览</button><button type="button" class="button primary" id="pages-publish-confirm" ${pages.publish_ready?'':'disabled'}>确认发布到 Pages</button></div>${pages.publish_ready?'':'<div class="admin-note">实际发布默认关闭。服务器需同时配置 SMART_MANUAL_PAGES_PUBLISH_ENABLED=1 与 SMART_MANUAL_GITHUB_TOKEN；令牌不会返回网页或写入日志。</div>'}</section>`}
...
function showAdminEditor(type,row={}) {
  ...
  $('#admin-editor-form').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),payload=Object.fromEntries(fd);try{
    if(type==='category')payload.sort_order=Number(payload.sort_order);
    if(type==='product'){payload.price=Number(payload.price);payload.cost=Number(payload.cost);payload.enabled=fd.has('enabled')?1:0;payload.featured=fd.has('featured')?1:0;payload.specs=JSON.parse(payload.specs||'{}');payload.capabilities=JSON.parse(payload.capabilities||'[]')}
    ...
    if(type==='document')payload.product_id=Number(payload.product_id);
    if(type==='hotspot'){payload.product_id=Number(payload.product_id);payload.sort_order=Number(payload.sort_order);payload.position={x:Number(payload.x),y:Number(payload.y)};delete payload.x;delete payload.y}
    const plural={category:'categories',product:'products',document:'documents',tutorial:'tutorials',hotspot:'hotspots',order:'orders',contact:'contacts'}[type];await adminApi(`/api/admin/${plural}${row.id?'/'+row.id:''}`,{method:row.id?'PATCH':'POST',body:JSON.stringify(payload)});dialog.close();state.products=[];state.categories=[];await ensureCatalog();toast(type==='contact'?'支持单已更新':type==='order'?'订单履约已更新':'内容已保存');if(location.hash==='#admin')renderAdmin();
  }catch(err){toast(err.message)}}; dialog.showModal();
}
function bindAdmin(){
  ...
  const pagesPublish=$('#pages-publish-confirm');if(pagesPublish)pagesPublish.onclick=async()=>{if(!confirm('确认把当前已启用商品和公开资料目录发布到 Pages 仓库？此操作会创建 GitHub 提交。'))return;pagesPublish.disabled=true;pagesPublish.textContent='正在发布…';try{const result=await adminApi('/api/admin/pages/publish',{method:'POST',body:JSON.stringify({confirm:true,message:`content: publish catalog ${new Date().toISOString().slice(0,10)}`})});toast(`Pages 目录已发布 · ${result.commit_sha.slice(0,12)}`);if(location.hash==='#admin')renderAdmin()}catch(err){toast(err.message)}finally{pagesPublish.disabled=false;pagesPublish.textContent='确认发布到 Pages'}};
