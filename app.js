...
addEventListener('load',()=>navigator.serviceWorker.register(sitePath('service-worker.js?v=1.30.3'),{updateViaCache:'none'}).catch(()=>{}));
}
...
function showAdminEditor(type,row={}) {
  ...
  $('#admin-editor-form').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),payload=Object.fromEntries(fd);try{
    ...
    if(type==='document'){
      ...
      if(file?.size){const asset=await uploadAdminAsset(payload.product_id,'document',payload.version||'1.0',file);payload.file_url=asset.asset_url;payload.file_size=formatBytes(asset.file_size);if(!payload.title.trim())payload.title=file.name}
    }
