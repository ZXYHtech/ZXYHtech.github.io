...
async function renderProducts(initialCategory = '') {
  ...
  const update = () => {
    ...
    const rows = state.products.filter(p => {const window=productFrequencyWindow(p);return (!category || p.category_code === category) && (!favoritesOnly||state.favorites.includes(p.id)) && (!q || `${p.name} ${p.model} ${p.description} ${JSON.stringify(p.specs)} ${(p.capabilities||[]).join(' ')}`.toLowerCase().includes(q)) && (!frequencyText||!Number.isFinite(frequency)||!window||(frequency>=window.min&&frequency<=window.max)) && (!priceText||p.price<=priceLimit)});
...
async function renderCompare() {
  ...
  app.innerHTML = `<div class="page" data-module="compare"><span class="eyebrow">PRODUCT COMPARISON</span><h1 class="page-title">把关键差异放在<br><em>同一张表里</em></h1>
    ...
    ${selected.length < 2 ? '<div class="empty panel">请至少添加两款产品进行对比，最多可同时选择三款。</div>' : `<div class="comparison-wrap" data-review-id="compare.table"><table class="comparison-table"><thead><tr><th>比较项目</th>${selected.map(p=>`<th><a href="#product/${p.slug}">${escapeHtml(p.model)}</a></th>`).join('')}</tr></thead><tbody>
      <tr><td>产品名称</td>${selected.map(p=>`<td>${escapeHtml(p.name)}</td>`).join('')}</tr><tr><td>销售价格</td>${selected.map(p=>`<td><b>${money(p.price)}</b></td>`).join('')}</tr><tr><td>产品类型</td>${selected.map(p=>`<td>${escapeHtml(p.category_code)}</td>`).join('')}</tr>
...
function ecosystemLinks(product){
  ...
  return `<section class="product-ecosystem" data-review-id="product.${escapeHtml(product.slug)}.ecosystem"><div class="section-head"><div><span class="eyebrow">PRODUCT ECOSYSTEM</span><h2>控制器、模块与上位机协同使用</h2></div><p>相关入口按实际组合关系连接</p></div><div class="ecosystem-grid">
    ...
    <a href="#zya1000"><span>DESKTOP APP</span><b>ZYA1000 上位机</b><p>设备发现、多设备同步、补偿数据、日志和自动化测试。</p><i>查看软件与说明书 →</i></a>
...
function scoreProduct(product,requirements){const window=productFrequencyWindow(product),frequencyOk=!window||(requirements.frequency>=window.min&&requirements.frequency<=window.max),taskCategories={'实验室测量':['attenuator','amplifier','divider'],'自动化测试':['attenuator'],'无线接收':['amplifier','filter'],'多通道分配':['divider','amplifier'],'干扰抑制':['filter','attenuator']},taskOk=(taskCategories[requirements.task]||[]).includes(product.category_code),budgetOk=product.price<=requirements.budget,reasons=[];let score=15;if(frequencyOk){score+=42;reasons.push(`覆盖 ${requirements.frequency} GHz`)}else{score-=35;reasons.push(`频率范围 ${window?.label||'未知'}`)}if(taskOk){score+=25;reasons.push(`适合${requirements.task}`)}else reasons.push(`并非${requirements.task}首选`);if(budgetOk){score+=12;reasons.push(`预算内余量 ${money(requirements.budget-product.price)}`)}else{score-=18;reasons.push(`超预算 ${money(product.price-requirements.budget)}`)}if(requirements.priority==='价格优先')score+=Math.max(0,14-product.price/100);if(requirements.priority==='易用优先')score+=Math.min(12,(product.capabilities||[]).length*2+(product.specs?.供电?0:4));if(requirements.priority==='性能优先'&&frequencyOk)score+=8;if(!frequencyOk)score=Math.min(score,39);return{product,score:Math.max(0,Math.min(99,Math.round(score))),frequencyOk,taskOk,budgetOk,reasons}}
...
async function renderAdmin() {
  ...
  const auditActionLabels={create:'新增',update:'更新',delete:'删除',retry:'重试',export:'导出',login:'登录',logout:'退出'};
  const auditEntityLabels={category:'分类',product:'商品',document:'文档',tutorial:'教程',hotspot:'热点',asset:'资源',order:'订单',contact:'支持单',annotation:'批注',session:'会话'};
...
document.addEventListener('click', e => {
  ...
  if(e.target.closest('[data-demo-download]')) toast('演示版本已定位文件位；接入真实文件后将直接下载。');
