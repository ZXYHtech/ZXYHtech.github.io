...
const state = {
  ...
  adminToken: sessionStorage.getItem('zya-admin-token') || '', adminAccount: null,
  adminTab: 'products',
...
function priceLabel(value){return Number(value)>0?`${money(value)} 起`:'联系询价'}
...
async function downloadFullBackup(){try{const ticket=await adminApi('/api/admin/backup/ticket',{method:'POST',body:'{}'});if(!ticket.download_url)throw new Error('服务器没有返回备份下载地址');const response=await fetch(apiPath(ticket.download_url));if(!response.ok)throw new Error('备份下载地址已过期，请重新点击下载');const blob=await response.blob(),url=URL.createObjectURL(blob),stamp=new Date().toISOString().replace(/[-:T]/g,'').slice(0,14),link=document.createElement('a');link.href=url;link.download=`zya-smart-manual-backup-${stamp}.zip`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);return true}catch(err){toast(err.message);return false}}
...
async function renderProducts(initialCategory = '') {
  await ensureCatalog();
  await loadAvailability();
  const categoryHero={controller:['CONTROL PLATFORM','控制射频模块，<br><em>也管理测试流程</em>','从本机操作到 ZYA1000 自动化，一个控制器连接多种射频功能模块。'],attenuator:['ATTENUATORS','精确设定信号，<br><em>快速得到目标衰减</em>','从固定衰减到 PE43711 数字程控，按工作频率、步进和供电版本选择。'],amplifier:['AMPLIFIERS','放大微弱信号，<br><em>保护整条测试链路</em>','覆盖 ZYP100 / ZYP101 系列，结合频段、增益、噪声与供电条件选择。'],divider:['POWER DIVIDERS','把一路信号，<br><em>稳定分配到多路</em>','按频率、路数、插损和隔离度选择功分与合路方案。'],filter:['FILTERS','从 LC 到 LTCC / SAW，<br><em>快速搭建滤波链路</em>','覆盖 ZYF100 低通成品与 LC、LTCC、SAW 滤波器测试板。'],switch:['RF SWITCHES','切换射频通道，<br><em>保持链路清晰可控</em>','按工作频率、隔离度、通道结构和控制电平选择。'],accessory:['RF ACCESSORIES','补齐接口与连接，<br><em>让测试链路可靠落地</em>','按连接器类型、方向、阻抗和机械规格选择射频配件。']}[initialCategory]||['PRODUCT CENTER','找到适合你链路的<br><em>射频产品</em>','控制器、衰减器、放大器、功分器、滤波器、射频开关和配件集中查找。'];
  ...
  const hasPublishedPrices=state.products.some(product=>Number(product.price)>0);
  app.innerHTML = `<div class="page ${controllerCategory?'controller-category-page':''}" data-module="products"><span class="eyebrow">${categoryHero[0]}</span><h1 class="page-title">${categoryHero[1]}</h1><p class="lead category-lead">${categoryHero[2]}</p>
    ...
    <div class="product-filter-panel ${controllerCategory?'controller-filter-panel':''}"><label class="search-box"><span>关键词</span><input id="product-search" placeholder="搜索型号、名称或用途"></label>${controllerCategory?'':`<label><span>工作频率</span><span class="product-filter-input"><input id="product-frequency" type="number" min="0.001" max="100" step="0.05" placeholder="不限"><i>GHz</i></span></label>${hasPublishedPrices?`<label><span>价格上限</span><select id="product-price"><option value="">不限</option><option value="400">¥400</option><option value="600">¥600</option><option value="800">¥800</option><option value="1000">¥1000</option></select></label>`:''}`}<label><span>排序方式</span><select id="product-sort"><option value="featured">推荐优先</option>${hasPublishedPrices?'<option value="price-asc">价格从低到高</option><option value="price-desc">价格从高到低</option>':''}<option value="model">型号名称</option></select></label><button type="button" class="button ghost compact" id="clear-product-filters">清除条件</button></div>
...
async function adminApi(path, options={}) {
  const response = await fetch(apiPath(path), {...options, headers:{'Content-Type':'application/json','Authorization':`Bearer ${state.adminToken}`}});
  const data = await response.json();
  if (!response.ok) {
    if (response.status === 401) { state.adminToken=''; sessionStorage.removeItem('zya-admin-token');syncRoleUI(); }
    if (data.code === 'password_change_required') { state.adminAccount={...(state.adminAccount||{}),must_change_password:true}; }
    throw new Error(data.error || '管理请求失败');
  }
  return data;
}
function adminLoginView() {
  app.innerHTML=`<div class="page" data-module="admin"><span class="eyebrow">OPERATIONS CONSOLE</span><h1 class="page-title">管理产品内容与<br><em>业务协同</em></h1><form class="panel admin-login" id="admin-login" data-review-id="admin.login"><h2>管理员登录</h2><p style="color:var(--muted)">请使用部署时配置的管理员账号和密码。</p><label>账号<input name="username" value="admin" required></label><label>密码<input name="password" type="password" required></label><button class="button primary" style="width:100%">进入运营后台</button></form></div>`;
  $('#admin-login').onsubmit=async e=>{e.preventDefault();try{const result=await api('/api/admin/login',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});state.adminToken=result.token;state.adminAccount=result;sessionStorage.setItem('zya-admin-token',result.token);syncRoleUI();result.must_change_password?adminPasswordChangeView(result):renderAdmin()}catch(err){toast(err.message)}};
}
function adminPasswordChangeView(account=state.adminAccount||{}) {
  app.innerHTML=`<div class="page" data-module="admin"><span class="eyebrow">SECURITY CHECK</span><h1 class="page-title">首次登录必须<br><em>修改临时密码</em></h1><form class="panel admin-login" id="admin-password-change"><h2>${escapeHtml(account.username||'管理员')} · 设置新密码</h2><p style="color:var(--muted)">至少 12 位，并包含大小写字母、数字或符号中的至少三类。修改后所有旧会话立即失效。</p><label>当前临时密码<input name="current_password" type="password" autocomplete="current-password" required></label><label>新密码<input name="new_password" type="password" autocomplete="new-password" minlength="12" required></label><label>确认新密码<input name="confirm_password" type="password" autocomplete="new-password" minlength="12" required></label><button class="button primary" style="width:100%">修改密码并重新登录</button><button class="button ghost" type="button" id="password-change-logout" style="width:100%">退出</button></form></div>`;
  $('#admin-password-change').onsubmit=async e=>{e.preventDefault();try{await adminApi('/api/admin/account/password',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});state.adminToken='';state.adminAccount=null;sessionStorage.removeItem('zya-admin-token');syncRoleUI();adminLoginView();toast('密码已修改，请使用新密码重新登录')}catch(err){toast(err.message)}};
  $('#password-change-logout').onclick=()=>{state.adminToken='';state.adminAccount=null;sessionStorage.removeItem('zya-admin-token');syncRoleUI();adminLoginView()};
}
...
async function renderAdmin() {
  ...
  await ensureCatalog();
  try {
    const accountResult=await adminApi('/api/admin/account');
    state.adminAccount=accountResult.user;
    if(state.adminAccount.must_change_password)return adminPasswordChangeView(state.adminAccount);
  ...
  const auditActionLabels={create:'新增',update:'更新',delete:'删除',retry:'重试',export:'导出',preview:'预览',publish:'发布',login:'登录',logout:'退出',password_change:'修改密码'};
  const auditEntityLabels={category:'分类',product:'商品',document:'文档',tutorial:'教程',hotspot:'热点',asset:'资源',order:'订单',contact:'支持单',annotation:'批注',pages_catalog:'Pages 目录',session:'会话',admin_user:'管理员账号'};
  ...
  const accountRows=d.users.map(user=>`<form class="admin-account-row" data-admin-user-form="${user.id}"><div><b>${escapeHtml(user.username)}</b><small>${user.id===d.account.id?'当前账号':user.last_login_at?`最后登录 ${escapeHtml(user.last_login_at)}`:'尚未登录'}</small></div><label>账号<input name="username" value="${escapeHtml(user.username)}" ${user.id===d.account.id?'readonly':''}></label><label>角色<select name="role" ${user.id===d.account.id?'disabled':''}><option value="admin" ${user.role==='admin'?'selected':''}>管理员</option><option value="owner" ${user.role==='owner'?'selected':''}>所有者</option></select></label><label class="account-check"><input name="enabled" type="checkbox" ${user.enabled?'checked':''} ${user.id===d.account.id?'disabled':''}>允许登录</label><label>临时密码<input name="temporary_password" type="password" minlength="12" placeholder="不修改请留空" ${user.id===d.account.id?'disabled':''}></label>${user.id===d.account.id?'<span class="status-tag on">当前会话</span>':'<button class="button ghost compact">保存账号</button>'}</form>`).join('');
  const accountPanel=`<div class="account-management-grid"><form class="panel form-grid" id="admin-own-password-form"><div><span class="eyebrow">MY ACCOUNT</span><h3>修改当前密码</h3><p>${escapeHtml(d.account.username)} · ${d.account.role==='owner'?'所有者':'管理员'}</p></div><label>当前密码<input name="current_password" type="password" required></label><label>新密码<input name="new_password" type="password" minlength="12" required></label><label>确认新密码<input name="confirm_password" type="password" minlength="12" required></label><div><button class="button primary">修改并重新登录</button></div></form>${d.account.role==='owner'?`<form class="panel form-grid" id="admin-user-create"><div><span class="eyebrow">NEW ADMIN</span><h3>新增管理员账号</h3><p>新账号首次登录必须修改临时密码。</p></div><label>账号<input name="username" pattern="[A-Za-z0-9_.-]{3,40}" required></label><label>角色<select name="role"><option value="admin">管理员</option><option value="owner">所有者</option></select></label><label>临时密码<input name="temporary_password" type="password" minlength="12" required></label><div><button class="button primary">创建账号</button></div></form>`:''}</div>${d.account.role==='owner'?`<div class="admin-account-list">${accountRows}</div>`:'<div class="admin-note">只有所有者可以创建、停用或重置其他管理员账号。</div>'}`;
  app.innerHTML=`<div class="page" data-module="admin"><div class="admin-role-banner"><b>管理员工作区</b><span>此区域包含上传、成本、订单、同步日志和验收工具，访客不可见。</span></div><div class="admin-head"><div><span class="eyebrow">OPERATIONS CONSOLE</span><h1 class="page-title">运营管理后台</h1></div><div><a class="button ghost" href="#analytics">经营分析</a> <span class="status-tag ${d.overview.inventory.ok?'on':'warn'}">库存：${escapeHtml(d.overview.inventory.mode)}</span> <button class="button ghost" id="admin-logout">退出</button></div></div>
    ...
    <div class="admin-shell"><nav class="admin-tabs">${[['categories','分类'],['products','商品'],['documents','文档'],['tutorials','教程'],['hotspots','产品热点'],['assets','资源文件'],['orders','订单'],['contacts','支持单'],['annotations','验收批注'],['integration','同步日志'],['audit','操作记录'],['accounts','账号安全'],['system','系统维护']].map(([k,n],i)=>`<button data-admin-tab="${k}" class="${i?'':'active'}">${n}</button>`).join('')}</nav>
      ...
      <section class="admin-pane" data-admin-pane="accounts"><div class="admin-toolbar"><div><h2>账号与密码安全</h2><p>管理后台账号、角色、登录状态和强制密码重置。</p></div></div>${accountPanel}</section>
...
function bindAdmin(){
  ...
  const ownPasswordForm=$('#admin-own-password-form');if(ownPasswordForm)ownPasswordForm.onsubmit=async e=>{e.preventDefault();try{await adminApi('/api/admin/account/password',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});state.adminToken='';state.adminAccount=null;sessionStorage.removeItem('zya-admin-token');syncRoleUI();adminLoginView();toast('密码已修改，请重新登录')}catch(err){toast(err.message)}};
  const createUserForm=$('#admin-user-create');if(createUserForm)createUserForm.onsubmit=async e=>{e.preventDefault();try{await adminApi('/api/admin/users',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});state.adminTab='accounts';toast('管理员账号已创建，首次登录必须改密');renderAdmin()}catch(err){toast(err.message)}};
  $$('[data-admin-user-form]').forEach(form=>{if(Number(form.dataset.adminUserForm)===Number(state.adminAccount?.id))return;form.onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),payload=Object.fromEntries(fd);payload.enabled=fd.has('enabled');try{await adminApi(`/api/admin/users/${form.dataset.adminUserForm}`,{method:'PATCH',body:JSON.stringify(payload)});state.adminTab='accounts';toast(payload.temporary_password?'临时密码已重置，旧会话已失效':'管理员账号已更新');renderAdmin()}catch(err){toast(err.message)}}});
  ...
  const backupButton=$('#download-full-backup');if(backupButton)backupButton.onclick=async()=>{backupButton.disabled=true;backupButton.textContent='正在生成一次性下载地址…';try{if(await downloadFullBackup())toast('完整备份已生成并开始下载')}finally{backupButton.disabled=false;backupButton.textContent='下载完整备份 ZIP'}};
