...
window.zyc100Story = function () {
  ...
  return `<section class="zyc-story" aria-label="ZYC100 产品介绍">
    ...
    <section class="zyc-configs"><div class="zyc-title"><span class="zyc-kicker">CHOOSE YOUR SETUP</span><h2>三种配置，按使用方式选择。</h2><p>两款组合均搭配 ZYE660 衰减模块；区别在于模块下方是否集成电池。</p></div><div class="zyc-config-grid">
      <article class="real-controller"><img src="${asset('real-thin-top.jpg')}" alt="组合状态中的 ZYC100 控制器实拍局部" width="1280" height="1280" loading="lazy"><span class="zyc-kicker">CONTROLLER</span><h3>单独控制器</h3><p>已有兼容模块，或希望自行搭配。控制器本体不包含射频衰减模块。</p><footer>不含 ZYE660 模块 · 询价</footer></article>
    ...
    </div><p class="zyc-note">页面直接使用实物照片裁切；单独控制器栏为组合状态中的控制器实拍局部，没有补画照片外结构。价格、库存及交期请咨询确认。</p></section>
