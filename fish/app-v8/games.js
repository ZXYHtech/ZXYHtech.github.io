const openGames={
  monadfish:{title:'MonadFish 中文版',icon:'🌌',url:'/fish/games/monadfish/play-v16-canvas.html',license:'MIT',repo:'KaimiEwl/fishing-game',note:'V16 Canvas 接管版：不再依赖旧人物资源名，直接按主画布中的真实船/人物绘制几何特征锁定并替换；人物、可见鱼竿和鱼线共用同一坐标链。',health:(w,d)=>{const v=w.__MONADFISH_V16_CHECK__?.();return{ready:w.__MONADFISH_LITE_READY__===true&&w.__MONADFISH_RELEASE__==='V16-CANVAS'&&w.__MONADFISH_V16_CANVAS__===true&&!!w.__MONADFISH_GAMEPLAY_V8__,ui:!!d.querySelector('button[aria-label="Cast line"],button[aria-label="Hook fish"],button[aria-label="抛竿"],button[aria-label="提竿"]'),state:!!v&&v.assetsReady===true&&v.replacementDraws>5&&v.replacing===true}}},
...
function games(){
  ...
  shell(`<div class="hero"><span class="eyebrow">开源小游戏 · 同源镜像</span><h1>只上经过实玩检查的游戏。</h1><div class="lead">MonadFish V16 不再根据打包后的图片文件名猜人物船：运行时直接锁定主 Canvas 中持续出现的船/人物绘制调用，只有实际替换帧持续增长才通过健康检查。</div></div>
...
function externalGame(id){
  ...
  const release=id==='monadfish'?'MFV16-CANVAS':'PLAYFIX4';
  ...
  const checkRuntime=(attempt=0)=>{
    try{
      ...
      if(ok){loading.style.display='none';hint.textContent='✅ V16 运行检查通过：主 Canvas 人物替换帧正在持续增长。';hint.className='game-hint loaded pass';return}
      if(attempt<80){setTimeout(()=>checkRuntime(attempt+1),250);return}
      loading.style.display='none';hint.textContent=`⚠️ 运行检查未通过：origin ${okOrigin?'✓':'×'} / script ${h.ready?'✓':'×'} / ui ${h.ui?'✓':'×'} / replacement-live ${h.state?'✓':'×'}`;hint.className='game-hint warn';
    }catch(err){
      if(attempt<80){setTimeout(()=>checkRuntime(attempt+1),250);return}
