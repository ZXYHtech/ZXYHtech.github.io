const openGames={
  monadfish:{title:'MonadFish 中文版',icon:'🌌',url:'/fish/games/monadfish/play-v14-hardperson.html',license:'MIT',repo:'KaimiEwl/fishing-game',note:'V14 人物硬兜底版：人物改为页面内联 SVG，完全绕过坏掉的人物位图；进入钓鱼页即固定可见，搏鱼时跟随方向摆动。',health:(w,d)=>({ready:w.__MONADFISH_LITE_READY__===true&&w.__MONADFISH_RELEASE__==='V14-HARDPERSON'&&w.__MONADFISH_V14_HARDPERSON__===true&&!!w.__MONADFISH_GAMEPLAY_V8__,ui:!!d.querySelector('button[aria-label="Cast line"],button[aria-label="Hook fish"],button[aria-label="抛竿"],button[aria-label="提竿"]'),state:!!d.querySelector('#mf-v14-angler')&&w.__MONADFISH_V14_CHECK__?.().personVisible===true})},
...
function games(){
  ...
  shell(`<div class="hero"><span class="eyebrow">开源小游戏 · 同源镜像</span><h1>只上经过实玩检查的游戏。</h1><div class="lead">MonadFish V14 不再依赖人物 WebP/背景图集：人物直接作为内联 SVG 放在页面最上层，默认就显示；健康检查也要求人物当前实际可见。</div></div>
...
function externalGame(id){
  ...
  const release=id==='monadfish'?'MFV14-HARDPERSON':'PLAYFIX4';
  ...
  const checkRuntime=(attempt=0)=>{
    try{
      ...
      if(ok){loading.style.display='none';hint.textContent='✅ V14 运行检查通过：内联人物当前真实可见。';hint.className='game-hint loaded pass';return}
